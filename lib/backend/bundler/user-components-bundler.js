...
async function build(admin, {
  ...
  if (_bundlerEnv.default === 'development') {
    bundlerOptions.babelConfig = false; // bundlerOptions.external = [];
    // bundlerOptions.globals = [];
  }

  const output = await (0, _bundler.default)(bundlerOptions);
  console.timeEnd('Bundler');
  return output.code; //   let jsOutput = output.code;
  //   if (output.map) {
  //     jsOutput += `
