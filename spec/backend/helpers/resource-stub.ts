...
const buildProperties = (): Array<BaseProperty> => {
  const normalProperty = new BaseProperty({
    path: 'normal',
    type: 'number',
  }) as any;
  const nestedProperty = new BaseProperty({
    path: 'nested',
    type: 'mixed',
  }) as any;
  const nested2Property = new BaseProperty({
    path: 'nested',
    type: 'mixed',
  }) as any;
  const arrayProperty = new BaseProperty({
    path: 'arrayed',
    type: 'string',
  }) as any;
  const arrayMixedProperty = new BaseProperty({
    path: 'arrayedMixed',
    type: 'mixed',
  }) as any;
...
export default (): BaseResource => ({
  ...
  getCurrentAdmin: sinon.stub().returns(expectedResult.currentAdmin),
  setCurrentAdmin: sinon.stub(),
  id: sinon.stub().returns(expectedResult.id),
  properties: sinon.stub().returns(expectedResult.properties),
  property: sinon
    .stub()
    .returns(new BaseProperty({ path: 'prop', type: 'string' })),
  ...
  decorate: sinon.stub(),
  roles: sinon.stub().returns(expectedResult.roles),
  permissions: sinon.stub().returns(expectedResult.permissions),
});
