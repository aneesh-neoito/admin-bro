...
import {
  AdminBroOptionsWithDefault,
  AdminBroOptions,
} from './admin-bro-options.interface';
...
import {
  ...
  Action,
} from './backend/actions/action.interface';
...
export const defaultOptions: AdminBroOptionsWithDefault = {
  ...
  impersonatePath: DEFAULT_PATHS.impersonatePath,
  impersonateLogoutPath: DEFAULT_PATHS.impersonateLogoutPath,
  loginPath: DEFAULT_PATHS.loginPath,
  signupPath: DEFAULT_PATHS.signupPath,
  forgotPasswordPath: DEFAULT_PATHS.forgotPasswordPath,
  changePasswordPath: DEFAULT_PATHS.changePasswordPath,
  databases: [],
  resources: [],
  dashboard: {},
  login: {},
...
class AdminBro {
  ...
  initI18n(): void {
    ...
    Object.getOwnPropertyNames(this.translateFunctions).forEach(
      (translateFunctionName) => {
        this[translateFunctionName] = this.translateFunctions[
          translateFunctionName
        ];
      },
    );
  ...
  static registerAdapter({
    ...
    if (Database.isAdapterFor && Resource.isAdapterFor) {
      ...
      throw new Error(
        'Adapter elements has to be a subclass of AdminBro.BaseResource and AdminBro.BaseDatabase',
      );
    }
  }
  ...
  async initialize(): Promise<void> {
    if (process.env.NODE_ENV !== 'development' && !(process.env.ADMIN_BRO_SKIP_BUNDLE === 'true')) {
      console.log('AdminBro initialize');
  ...
  async renderLogin({ action, errorMessage }): Promise<string> {
    if (this.options.renderLogin) {
      return this.options.renderLogin(this, { action, errorMessage });
    }

    // if (process.env.NODE_ENV !== 'production') {
    //   await userComponentsBundler(this, { write: true, watch: true });
    // }
  ...
  public static bundle(
    ...
    if (src[0] === '/') {
      ...
      if (!pathNode8 && !pathNode10) {
        throw new Error(
          'STACK does not have a file url. Check out if the node version >= 8',
        );
      }
      const executionPath =
        (pathNode8 && pathNode8[1]) || (pathNode10 && pathNode10[1]);
