...
class ActionDecorator {
  ...
  async invokeBeforeHook(
    ...
    if (Array.isArray(this.action.before)) {
      return (this.action.before as Array<Before>).reduce(
        (prevPromise, hook) =>
          prevPromise.then((modifiedRequest) => hook(modifiedRequest, context)),
  ...
  async invokeHandler(
    ...
    if (Array.isArray(this.action.handler)) {
      return (this.action.handler as Array<
        ActionHandler<ActionResponse>
      >).reduce(
        (prevPromise, handler) =>
          prevPromise.then(() => handler(request, response, context)),
        Promise.resolve({}),
      );
    }
    throw new ConfigurationError(
  ...
  async invokeAfterHook(
    ...
    if (Array.isArray(this.action.after)) {
      return (this.action.after as Array<After<ActionResponse>>).reduce(
        (prevPromise, hook) =>
  ...
  is(
    what: 'isAccessible' | 'isVisible',
    currentAdmin?: CurrentAdmin,
    record?: BaseRecord,
  ): boolean {
    if (!['isAccessible', 'isVisible'].includes(what)) {
      throw new Error(
        `'what' has to be either "isAccessible" or "isVisible". You gave ${what}`,
      );
    }
    let isAction;
  ...
  canInvokeAction(context: ActionContext): boolean {
    ...
    if (
      records &&
      !records.find(
        (bulkRecord) => !this.isAccessible(currentAdmin, bulkRecord),
      )
    ) {
      return true;
    }
  ...
  toJSON(currentAdmin?: CurrentAdmin): ActionJSON {
    ...
    return {
      ...
      guard: this.action.guard
        ? this._admin.translateMessage(this.action.guard, resourceId)
        : '',
