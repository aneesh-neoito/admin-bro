...
class ResourceDecorator {
  ...
  /**
   * Returns the roles for the resource.
   * @return {string[]} resource id
   */
   roles(): string[] {
    return this.options.roles || this._resource.roles();
  }

  /**
   * Returns the permissions for the resource.
   * @return {string[]} resource id
   */
   permissions(): string[] {
    return this.options.permissions || this._resource.permissions();
  }
  ...
  getProperties({
    ...
    if (
      where &&
      this.options[whereProperties] &&
      this.options[whereProperties].length
    ) {
  ...
  getHref(currentAdmin?: CurrentAdmin): string | null {
    ...
    if (
      this.resourceActions(currentAdmin).find(
  ...
  toJSON(currentAdmin?: CurrentAdmin): ResourceJSON {
    ...
    const flattenPropertiesJSON = Object.keys(flattenProperties).reduce(
      (memo, key) => ({
        ...memo,
    ...
    return {
      ...
      resourceActions: this.resourceActions(currentAdmin).map((ra) =>
        ra.toJSON(currentAdmin),
      ),
      actions: Object.values(this.actions).map((action) =>
        action.toJSON(currentAdmin),
      ),
