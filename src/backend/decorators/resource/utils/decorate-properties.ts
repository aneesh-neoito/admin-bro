...
const organizeNestedProperties = (
  ...
  const rootPropertyKeys = Object.keys(properties).filter((key) => {
    ...
    const parts = pathToParts(property.propertyPath, { skipArrayIndexes: true })
      .reverse()
      .splice(1);
    if (parts.length) {
      const mixedPropertyPath = parts.find(
        (part) => properties[part] && properties[part].type() === 'mixed',
      );
...
export function decorateProperties(
  ...
  const virtualProperties = decorateVirtualProperties(
    dbProperties,
    admin,
    decorator,
  );
