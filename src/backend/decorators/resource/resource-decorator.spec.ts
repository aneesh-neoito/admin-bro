...
import resourceStub, {
  expectedResult,
} from '../../../../spec/backend/helpers/resource-stub';
...
describe('ResourceDecorator', function () {
  ...
  describe('#getProperties', function () {
    context('all properties are visible', function () {
      ...
      it('returns all properties when limit is not given', function () {
        ...
        expect(decorator.getProperties({ where: 'list' })).to.have.lengthOf(
          expectedResult.properties.length,
        );
      });
      ...
      it('returns only showProperties from options if they were given', function () {
        ...
        expect(decorator.getProperties({ where: 'show' })).to.have.lengthOf(1);
      });
    });
  });
  ...
  describe('#getPropertyByKey', function () {
    ...
    it('returns property by giving its key', function () {
      ...
      expect(decorator.getPropertyByKey(propertyPath)).to.be.an.instanceof(
        PropertyDecorator,
      );
    });
    ...
    it('returns mixed property', function () {
      const propertyPath = expectedResult.properties
        .find((p) => p.type() === 'mixed')
        ?.path();
    ...
    it('returns nested property under mixed', function () {
      const property = expectedResult.properties.find(
        (p) => p.type() === 'mixed',
      ) as BaseProperty;
      const nested1Property = property
        ?.subProperties()
        .find((p) => p.type() !== 'mixed') as BaseProperty;
      const path = [property.path(), nested1Property.path()].join('.');

      const decoratedProperty = decorator.getPropertyByKey(
        path,
    ...
    it('returns nested property under 2 level nested mixed', function () {
      ...
      const path = [
        property.path(),
        nested1Property.path(),
        nested2Property.path(),
      ].join('.');
    ...
    it('returns property when it is an array', function () {
      const arrayProperty = expectedResult.properties.find((p) =>
        p.isArray(),
      ) as BaseProperty;
    ...
    it('returns property when it is an nested array', function () {
      const arrayProperty = expectedResult.properties.find(
        (p) => p.isArray() && p.type() === 'mixed',
      ) as BaseProperty;
      ...
      expect(decoratedProperty).to.be.an.instanceof(PropertyDecorator);
      expect(decoratedProperty.propertyPath).to.eq(
        [arrayProperty.path(), nested1Property.path()].join('.'),
      );
    });
  });
  ...
  describe('#toJSON', function () {
    ...
    it('passes current admin to the resourceActions', function () {
      const resourceActionsSpy = sinon.spy(
        ResourceDecorator.prototype,
        'resourceActions',
      );
