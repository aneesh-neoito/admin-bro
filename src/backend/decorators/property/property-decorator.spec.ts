...
describe('PropertyDecorator', () => {
  ...
  beforeEach(() => {
    ...
    stubbedAdmin.translateProperty = sinon
      .stub()
      .returns(translatedProperty) as any;
    args = {
      property,
      admin: stubbedAdmin,
      resource: { id: () => 'someId' } as ResourceDecorator,
    };
  });
  ...
  describe('#isVisible', () => {
    it('passes execution to BaseProperty.isVisible for list when no options are specified', () => {
      expect(new PropertyDecorator(args).isVisible('list')).to.equal(
        property.isVisible(),
      );
    });
    ...
    it('passes execution to BaseProperty.isEditable for edit when no options are specified', () => {
      ...
      expect(new PropertyDecorator(args).isVisible('edit')).to.equal(
        property.isEditable(),
      );
    });
  ...
  describe('#reference', () => {
    ...
    it('returns model from AdminBro for reference name in properties', () => {
      ...
      expect(args.admin.findResource).to.have.been.calledWith(
        rawReferenceValue,
      );
    });
  ...
  describe('#subProperties', () => {
    ...
    beforeEach(() => {
      ...
      sinon
        .stub(property, 'subProperties')
        .returns([new BaseProperty({ path: subPropertyName, type: 'string' })]);
    ...
    it('returns the array of decorated properties', () => {
      ...
      expect(propertyDecorator.subProperties()[0]).to.be.instanceOf(
        PropertyDecorator,
      );
    });
