...
import BaseProperty, {
  PropertyType,
} from '../../adapters/property/base-property';
...
class PropertyDecorator {
   ...
   */
  label(): string {
    return this._admin.translateProperty(
      this.propertyPath,
      this._resource.id(),
    );
  }
  ...
  isVisible(where: PropertyPlace): boolean {
    if (
      typeof this.options.isVisible === 'object' &&
      this.options.isVisible !== 'null'
    ) {
  ...
  toJSON(where?: PropertyPlace): BasePropertyJSON {
    return {
      ...
      custom:
        typeof this.options.custom === 'undefined' ? {} : this.options.custom,
