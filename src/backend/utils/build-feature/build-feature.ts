...
type MissingKeys = Required<
  Omit<ResourceOptions, BasicOption | ListOption | 'actions' | 'properties' | 'roles' | 'permissions'>
>;
...
const mergeResourceOptions = (
  ...
  if (oldOptions.actions || newOptions.actions) {
    options.actions = Object.keys(newOptions.actions || {}).reduce(
      (memo, actionName) => {
        const action = (newOptions.actions || {})[
          actionName
        ] as Action<ActionResponse>;
...
const buildFeature = (options: ResourceOptions = {}): FeatureType => (
  prevOptions: ResourceOptions = {},
): ResourceOptions => mergeResourceOptions(prevOptions, options);
