...
export class NoDatabaseAdapterError extends Error {
  ...
  constructor(database: string) {
    const message =
      'There are no adapters supporting one of the database you provided';
    super(message);
...
export class ResourcesFactory {
  ...
  buildResources({ databases, resources }): Array<BaseResource> {
    ...
    const databaseResources = this._convertDatabases(databases).filter(
      (dr) =>
        !optionsResources.find(
          (optionResource) => optionResource.resource.id() === dr.id(),
        ),
    );
    ...
    return resources.map((rawResource) => {
      ...
      return {
        resource: resourceAdapter
          ? new resourceAdapter.Resource(resourceObject)
          : resourceObject,
  ...
  _decorateResources(
    ...
    return resources.map((resourceObject) => {
      ...
      const optionsFromFeatures = features.reduce(
