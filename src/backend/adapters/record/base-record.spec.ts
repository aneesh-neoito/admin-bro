...
import ValidationError, {
  PropertyErrors,
} from '../../utils/errors/validation-error';
  ...
  describe('#get', function () {
    context('record with nested parameters', function () {
      ...
      it('returns deepest field when all up-level keys are given', function () {
        expect(record.get('nested1level.nested2level.nested3level')).to.equal(
          nested3level,
        );
      });
  ...
  describe('#save', function () {
    ...
    beforeEach(function () {
      resource = sinon.createStubInstance(BaseResource, {
        ...
        create: sinon
          .stub<[Record<string, any>], Promise<ParamsType>>()
          .resolves(newParams),
        update: sinon
          .stub<[string, Record<string, any>], Promise<ParamsType>>()
          .resolves(newParams),
      });
    });
      ...
      resource = sinon.createStubInstance(BaseResource, {
        ...
        decorate: sinon.stub<[], ResourceDecorator>().returns(
          (sinon.createStubInstance(ResourceDecorator, {
            recordActions: sinon
              .stub<[BaseRecord], ActionDecorator[]>()
              .returns([]),
            bulkActions: sinon
              .stub<[BaseRecord], ActionDecorator[]>()
              .returns([]),
