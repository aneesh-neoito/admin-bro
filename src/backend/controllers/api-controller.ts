...
class ApiController {
  ...
  async getActionContext(request: ActionRequest): Promise<ActionContext> {
    ...
    resource.setCurrentAdmin(this.currentAdmin);
  ...
  async search(
    ...
    return this.resourceAction(
      request,
      response,
    ) as Promise<SearchActionResponse>;
  }
  ...
  async recordAction(
    ...
    if (!recordId) {
      throw new NotFoundError(
        ['You have to pass recordId to the recordAction'].join('\n'),
        'Action#handler',
      );
    }
    ...
    const jsonWithRecord = await actionContext.action.handler(
      request,
      response,
      actionContext,
    );
    ...
    const isValidRecord = !!(
      jsonWithRecord &&
      jsonWithRecord.record &&
      jsonWithRecord.record.recordActions
    );
    const anErrorWasHandled =
      jsonWithRecord &&
      jsonWithRecord.notice &&
      jsonWithRecord.notice.type === 'error';
  ...
  async dashboard(request: any, response: any): Promise<any> {
    const h = new ViewHelpers(this._admin);
    const handler =
      this._admin.options.dashboard && this._admin.options.dashboard.handler;
    if (handler) {
