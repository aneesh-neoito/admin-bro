...
import recordToFormData, {
  FORM_VALUE_EMPTY_OBJECT,
  FORM_VALUE_NULL,
  FORM_VALUE_EMPTY_ARRAY,
} from './params-to-form-data';
...
describe('recordToFormData', function () {
  ...
  it('converts objects to const', async function () {
    const record = await factory.build<RecordJSON>('RecordJSON', {
      params: {
        [propertyKey]: {},
      },
    });

    expect(recordToFormData(record.params).get(propertyKey)).to.equal(
      FORM_VALUE_EMPTY_OBJECT,
    );
  });
