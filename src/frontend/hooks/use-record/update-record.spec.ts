...
describe('updateRecord', function () {
  ...
  it('flattens value when object is given', function () {
    ...
    expect(updatedRecord.params[`${newPropertyName}.sub1`]).to.equal(
      value.sub1,
    );
    expect(updatedRecord.params[`${newPropertyName}.sub2`]).to.equal(
      value.sub2,
    );
  });
  ...
  it('removes all previous array values also if they were flatten', function () {
    ...
    const updatedRecord = update(previousRecord);
    const unflattenParams = unflatten<Record<string, any>, any>(
  ...
  it('clears the populated value if it exists when no ref is given', function () {
    ...
    expect(update(previousRecord).populated[populatedPropertyName]).to.be
      .undefined;
  });
  ...
  it('deletes the entire property when undefined was given', function () {
    ...
    expect(
      Object.keys(updatedRecord.params).find((key) => key === propertyName),
    ).to.be.undefined;
  });
