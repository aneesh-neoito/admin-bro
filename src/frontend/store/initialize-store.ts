...
import {
  getBranding,
  getAssets,
} from '../../backend/utils/options-parser/options-parser';
...
export const initializeStore = async (
  ...
  store.dispatch(
    initializePaths({ loginPath, signupPath, forgotPasswordPath, changePasswordPath, logoutPath, impersonatePath, impersonateLogoutPath, rootPath, assetsCDN }),
  );
  ...
  store.dispatch(
    initializeVersions({
      ...
      admin:
        admin.options.version && admin.options.version.admin
          ? adminVersion
          : undefined,
    }),
  );
