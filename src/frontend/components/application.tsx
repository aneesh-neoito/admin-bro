...
import { Box, Overlay, Reset, cssClass } from '@admin-bro/design-system'
...
const App: React.FC = () => {
  ...
  return (
    <React.Fragment>
      ...
      <GlobalStyle />
      <Switch>
        <Route path={publicPageUrl} exact component={Page} />
        <Route>
          <Box height="100%" flex>
            {sidebarVisible ? (
              <Overlay
            ...
            <Box flex flexGrow={1} flexDirection="column" overflowY="auto" bg="bg">
              ...
              <Box className={cssClass(['Main'])} flex flexGrow={1}>
                <Switch>
