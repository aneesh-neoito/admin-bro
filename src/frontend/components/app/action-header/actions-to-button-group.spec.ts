...
describe('actionsToButtonGroup', () => {
  ...
  context('flat actions (no nesting)', () => {
    beforeEach(async () => {
      actions = await factory.buildMany<ActionJSON>(
        'ActionJSON',
        actionsCount,
        {
          actionType: 'record',
  ...
  context('nested actions', () => {
    ...
    beforeEach(async () => {
      rootActions = {
        normal: await factory.build<ActionJSON>('ActionJSON', {
          actionType: 'record',
        }),
        publish: await factory.build<ActionJSON>('ActionJSON', {
          actionType: 'record',
          name: 'publish',
        }),
        export: await factory.build<ActionJSON>('ActionJSON', {
          actionType: 'record',
          name: 'publish',
        }),
      };
      actionsPublish = await factory.buildMany<ActionJSON>(
        'ActionJSON',
        actionsCount,
        {
          actionType: 'record',
      ...
      actionsExport = await factory.buildMany<ActionJSON>(
        'ActionJSON',
        actionsCount,
        {
          actionType: 'record',
