...
const translate = (
  ...
  const realOptions: TOptions =
    (typeof resourceId === 'string' ? options : resourceId) || {};
...
export const createFunctions = (i18n: I18n): TranslateFunctions => {
  ...
  const translateProperty: TranslateFunction = (
    propertyName,
    resourceId,
    options,
  ) => translate(i18n, 'properties', propertyName, resourceId, options);

  const translateMessage: TranslateFunction = (
    messageName,
    resourceId,
    options,
