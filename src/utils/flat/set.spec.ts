...
describe('module:flat.set', () => {
  ...
  context('passing basic types', () => {
    ...
    it('sets null', () => {
      expect(set(params, newPropertyName, null)).to.have.property(
        newPropertyName,
        null,
      );
    });
  ...
  context('value is undefined', () => {
    ...
    it('does not set any new key', () => {
      expect(Object.keys(newParams).length).to.eq(
  ...
  context(
    ...
    () => {
      ...
      it('removes root property from keys', () => {
        ...
        expect(Object.keys(newParams)).not.to.include(
          newNestedNullKey.split('.')[0],
        );
      });
